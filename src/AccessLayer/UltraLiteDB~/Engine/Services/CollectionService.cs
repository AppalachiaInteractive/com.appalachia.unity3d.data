...
namespace UltraLiteDB
{
    internal class CollectionService
    {
        ...
        {
            if (string.IsNullOrEmpty(name)) throw new ArgumentNullException(nameof(name));
            if (!CollectionPage.NamePattern.IsMatch(name)) throw DataException.InvalidFormat(name);
            ...
            {
                throw DataException.CollectionLimitExceeded(CollectionPage.MAX_COLLECTIONS_SIZE);
            }
