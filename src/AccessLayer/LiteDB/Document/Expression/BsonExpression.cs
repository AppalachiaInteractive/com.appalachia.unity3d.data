...
namespace LiteDB
{
    ...
    /// <summary>
    ///     Compile and execute string expressions using BsonDocuments. Used in all document manipulation (transform, filter, indexes, updates). See
    ...
    public sealed class BsonExpression
    {
        ...
        /// </summary>
        internal bool IsANY => IsPredicate && Expression.ToString().Contains("_ANY");
        ...
        /// </summary>
        internal bool IsIndexable => (Fields.Count > 0) && IsImmutable && (Parameters.Count == 0);
        ...
        /// </summary>
        internal IEnumerable<BsonValue> Execute(
            IEnumerable<BsonDocument> source,
            BsonDocument root,
            BsonValue current,
            Collation collation)
        ...
        /// <summary>
        ///     Execute scalar expression over single document and return a single value (or BsonNull when empty). Throws exception if expression are not scalar
        ///     expression
        /// </summary>
        ...
        /// <summary>
        ///     Execute scalar expression over multiple documents and return a single value (or BsonNull when empty). Throws exception if expression are not
        ///     scalar expression
        /// </summary>
        ...
        ///     Execute expression and returns IEnumerable values - returns NULL if no elements
        /// </summary>
        internal BsonValue ExecuteScalar(
            IEnumerable<BsonDocument> source,
            BsonDocument root,
            BsonValue current,
            Collation collation)
        ...
        private static ConcurrentDictionary<string, BsonExpressionEnumerableDelegate> _cacheEnumerable =
            new ConcurrentDictionary<string, BsonExpressionEnumerableDelegate>();

        private static ConcurrentDictionary<string, BsonExpressionScalarDelegate> _cacheScalar =
            new ConcurrentDictionary<string, BsonExpressionScalarDelegate>();
        ...
        /// </summary>
        internal static BsonExpression Create(
            Tokenizer tokenizer,
            BsonExpressionParserMode mode,
            BsonDocument parameters)
        {
            if (tokenizer == null)
            {
        ...
        /// </summary>
        internal static BsonExpression ParseAndCompile(
            Tokenizer tokenizer,
            BsonExpressionParserMode mode,
            BsonDocument parameters,
            DocumentScope scope)
        {
            ...
            var expr = mode == BsonExpressionParserMode.Full
                ? BsonExpressionParser.ParseFullExpression(tokenizer, context, parameters, scope)
                : mode == BsonExpressionParserMode.Single
                    ? BsonExpressionParser.ParseSingleExpression(tokenizer, context, parameters, scope)
                    : mode == BsonExpressionParserMode.SelectDocument
                        ? BsonExpressionParser.ParseSelectDocumentBuilder(tokenizer, context, parameters)
        ...
        internal static void Compile(BsonExpression expr, ExpressionContext context)
        {
            ...
            if (expr.IsScalar)
            {
                var cached = _cacheScalar.GetOrAdd(
                    expr.Source,
                    s =>
                    {
                        var lambda = Expression.Lambda<BsonExpressionScalarDelegate>(
                            expr.Expression,
                            context.Source,
                            context.Root,
                            context.Current,
            ...
            {
                var cached = _cacheEnumerable.GetOrAdd(
                    expr.Source,
                    s =>
                    {
                        var lambda = Expression.Lambda<BsonExpressionEnumerableDelegate>(
                            expr.Expression,
                            context.Source,
                            context.Root,
                            context.Current,
        ...
        /// </summary>
        private static Dictionary<string, MethodInfo> _methods = typeof(BsonExpressionMethods)
                                                                .GetMethods(
        ...
        /// </summary>
        private static Dictionary<string, MethodInfo> _functions = typeof(BsonExpressionFunctions)
                                                                  .GetMethods(
                                                                       BindingFlags.Public |
                                                                       BindingFlags.Static
                                                                   )
