...
namespace LiteDB
{
    ...
    ///     - Entity class must have Id property, [ClassName]Id property or [DataId] attribute
    ...
    public partial class BsonMapper
    {
        ...
        protected virtual EntityMapper BuildEntityMapper(Type type)
        {
            ...
            var idAttr = typeof(DataIdAttribute);
            var ignoreAttr = typeof(DataIgnoreAttribute);
            var fieldAttr = typeof(DataFieldAttribute);
            var dbrefAttr = typeof(DataRefAttribute);
            ...
            foreach (var memberInfo in members)
            {
                ...
                // check if property has [DataField]
                var field = (DataFieldAttribute)CustomAttributeExtensions.GetCustomAttributes(memberInfo, fieldAttr, true).FirstOrDefault();

                // check if property has [DataField] with a custom field name
                ...
                // check if property has [DataId] to get with was setted AutoId = true
                var autoId = (DataIdAttribute)CustomAttributeExtensions.GetCustomAttributes(memberInfo, idAttr, true).FirstOrDefault();
                ...
                // check if property has [DataRef]
                var dbRef = (DataRefAttribute)CustomAttributeExtensions.GetCustomAttributes(memberInfo, dbrefAttr, false).FirstOrDefault();
        ...
        protected virtual MemberInfo GetIdMember(IEnumerable<MemberInfo> members)
        {
            return Reflection.SelectMember(members,
                x => CustomAttributeExtensions.IsDefined(x, typeof(DataIdAttribute), true),
        ...
        /// - Look if contains [DataCtor] attribute
        ...
        protected virtual CreateObject GetTypeCtor(EntityMapper mapper)
        {
            ...
            var ctor =
                ctors.FirstOrDefault(x => x.GetCustomAttribute<DataCtorAttribute>() != null && x.GetParameters().All(p => Reflection.ConvertType.ContainsKey(p.ParameterType) || _basicTypes.Contains(p.ParameterType) || p.ParameterType.GetTypeInfo().IsEnum)) ??
