...
namespace LiteDB.Engine
{
    ...
    internal class EnginePragmas
    {
        ...
        public EnginePragmas(HeaderPage headerPage)
        {
            ...
            _pragmas = new Dictionary<string, Pragma>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                [Engine.Pragmas.COLLATION] = new Pragma
                {
                    ...
                    Validate = (v, h) => { throw new DataException(0, "Pragma COLLATION is read only. Use Rebuild options."); },
                    Write = (b) =>
                    {
                ...
                [Engine.Pragmas.TIMEOUT] = new Pragma
                {
                    ...
                    Validate = (v, h) => { if (v <= 0) throw new DataException(0, "Pragma TIMEOUT must be greater than zero"); },
                ...
                [Engine.Pragmas.LIMIT_SIZE] = new Pragma
                {
                    ...
                    Validate = (v, h) =>
                    {
                        if (v < 4 * PAGE_SIZE) throw new DataException(0, "Pragma LIMIT_SIZE must be at least 4 pages (32768 bytes)");
                        if (h != null && v.AsInt64 < (h.LastPageID + 1) * Constants.PAGE_SIZE) throw new DataException(0, "Pragma LIMIT_SIZE must be greater or equal to the current file size");
                    },
                ...
                [Engine.Pragmas.CHECKPOINT] = new Pragma
                {
                    ...
                    Validate = (v, h) => { if (v < 0) throw new DataException(0, "Pragma CHECKPOINT must be greater or equal to zero"); },
        ...
        public BsonValue Get(string name)
        {
            ...
            throw new DataException(0, $"Pragma `{name}` not exist");
        }
