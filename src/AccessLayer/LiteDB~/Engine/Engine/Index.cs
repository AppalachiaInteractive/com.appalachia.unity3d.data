...
namespace LiteDB.Engine
{
    public partial class LiteEngine
    {
        ...
        public bool EnsureIndex(string collection, string name, BsonExpression expression, bool unique)
        {
            ...
            if (name.Length > INDEX_NAME_MAX_LENGTH) throw DataException.InvalidIndexName(name, collection, "MaxLength = " + INDEX_NAME_MAX_LENGTH);
            if (!name.IsWord()) throw DataException.InvalidIndexName(name, collection, "Use only [a-Z$_]");
            if (name.StartsWith("$")) throw DataException.InvalidIndexName(name, collection, "Index name can't starts with `$`");
            if (expression.IsScalar == false && unique) throw new DataException(0, "Multikey index expression do not support unique option");
            ...
            return this.AutoTransaction(transaction =>
            {
                ...
                if (current != null)
                {
                    ...
                    if (current.Expression != expression.Source) throw DataException.IndexAlreadyExist(name);
        ...
        public bool DropIndex(string collection, string name)
        {
            ...
            if (name == "_id") throw DataException.IndexDropId();
