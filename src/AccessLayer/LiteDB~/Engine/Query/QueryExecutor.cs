            ...
            IEnumerable<BsonDocument> RunQuery()
            {
                ...
                try
                {
                    ...
                    foreach (var doc in pipe.Pipe(nodes, queryPlan))
                    {
                        ...
                        if (transaction.State != TransactionState.Active) throw new DataException(0, $"There is no more active transaction for this cursor: {_cursor.Query.ToSQL(_cursor.Collection)}");
