...
using Appalachia.Data.Core.Bson.Document.Bson;
...
namespace LiteDB.Engine
{
    ...
    internal class FileReaderV7 : IFileReader
    {
        ...
        public FileReaderV7(Stream stream, string password)
        {
            ...
            {
                throw new DataException(0, "Current data file requires password");
            }
            else if (password != null)
            {
                if (_header["salt"].AsBinary.IsFullZero())
                {
                    throw new DataException(0, "Current data file has no encryption - do not use password");
                }
                ...
                {
                    throw new DataException(0, "Invalid password");
                }
            }
                ...
                foreach(var node in indexPage["nodes"].AsArray)
                {
                    ...
                    if (dataBlock["pageID"].AsInt32 != -1)
                    {
                        ...
                        if (collection == "_chunks")
                        {
                            ...
                            if (!int.TryParse(parts[1], out var n)) throw DataException.InvalidFormat("_id");
        ...
        private BsonDocument ReadPage(uint pageID)
        {
            ...
            if (page["pageType"] == 1)
            {
                ...
                {
                    throw DataException.InvalidDatabase();
                }
