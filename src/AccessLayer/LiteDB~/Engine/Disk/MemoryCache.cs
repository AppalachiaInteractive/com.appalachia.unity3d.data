                ...
                foreach (var key in readables)
                {
                    ...
                    if (page.ShareCounter > 0)
                    {
                        ...
                        if (!_readable.TryAdd(key, page))
                        {
                            ...
                            throw new DataException(0, "MemoryCache: removed in-use memory page. This situation has no way to fix (yet). Throwing exception to avoid database corruption. No other thread can read/write from database now.");
                        }
                    }
                    else
                    {
