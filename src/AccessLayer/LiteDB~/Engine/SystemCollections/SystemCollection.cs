...
namespace LiteDB.Engine
{
    ...
    internal class SystemCollection
    {
        ...
        /// </summary>
        public virtual int Output(IEnumerable<BsonDocument> source, BsonValue options) => throw new DataException(0, $"{_name} do not support as output collection");
        ...
        protected static BsonValue GetOption(BsonValue options, string key, BsonValue defaultValue)
        {
            if (options != null && options.IsDocument)
            {
                if (options.AsDocument.TryGetValue(key, out var value))
                {
                    ...
                    {
                        throw new DataException(0, $"Parameter `{key}` expect {defaultValue.Type} value type");
                    }
                }
                else
                {
