...
namespace LiteDB.Engine
{
    internal class CollectionService
    {
        ...
        {
            if (Encoding.UTF8.GetByteCount(name) > header.GetAvailableCollectionSpace()) throw DataException.InvalidCollectionName(name, "There is no space in header this collection name");
            if (!name.IsWord()) throw DataException.InvalidCollectionName(name, "Use only [a-Z$_]");
            if (name.StartsWith("$")) throw DataException.InvalidCollectionName(name, "Collection can't starts with `$` (reserved for system collections)");
        }
